...
export default class SyncResolver {
    ...
    private lstat(absPath: string): fs.Stats | null {
        ...
        const lstat =
            this.fileSystem.lstatSync(absPath, {throwIfNoEntry: false}) ?? null;

        if (!lstat) {
            const stat =
                this.fileSystem.statSync(absPath, {throwIfNoEntry: false}) ??
                null;
    ...
    private realpath(absPath: string): RealpathResult | null {
        ...
        while ((cursor = stack.pop())) {
            ...
            if (cachedSymlink) {
                const targetRealpath = path.join(
                    cachedSymlink,
                    absPath.slice(cursor.length),
                );
    ...
    private tryToResolveInDir(absPath: string): string | null {
        const resolvedFromPackageJSON =
            this.tryToResolveFromPackageJSON(absPath);
    ...
    private tryToResolveFromPackageJSON(absPath: string): string | null {
        ...
        let resolvedFromPackageJSON =
            this.resolveFromPackageJSONCache.get(packageJSONPath);
        ...
        resolvedFromPackageJSON = this.resolveFromPackageJSONCache.get(
            realpath.abs,
        );
        ...
        let packageJSON = JSON.parse(
            this.fileSystem.readFileSync(realpath.abs, 'utf-8'),
        );
        const modifierResult = this.packageJSONModifier?.(
            realpath.abs,
            packageJSON,
        );
    ...
    private resolvePackage(basedir: string, request: string): string | null {
        const cached = this.resolveNodeModuleCache.get(
        ...
        while ((cursor = stack.pop())) {
            passed.add(cursor);
            nearestNodeModule = this.resolveNodeModuleCache.get(
                path.resolve(cursor, request),
            );
            ...
            for (const modulesDir of this.modules) {
                ...
                if (resolvedFile) {
                    for (const el of passed) {
                        this.resolveNodeModuleCache.set(
                            path.resolve(el, request),
                            resolvedFile,
                        );
                    }
